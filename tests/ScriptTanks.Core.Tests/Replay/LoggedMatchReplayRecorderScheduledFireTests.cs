...
public sealed class LoggedMatchReplayRecorderScheduledFireTests
{
    ...
    public void RecordUntilEnd_WithScheduledFire_RejectsNullInitialState()
    {
        Assert.Throws<ArgumentNullException>(
            () => LoggedMatchReplayRecorder.RecordUntilEnd(
                initialState: null!,
                maxTicks: 100,
    ...
    public void RecordUntilEnd_WithScheduledFire_RejectsScheduleEarlierThanInitialTick()
    {
        ...
        ArgumentException ex = Assert.Throws<ArgumentException>(
            () => LoggedMatchReplayRecorder.RecordUntilEnd(state, maxTicks: 100, schedule));
    ...
    public void RecordUntilEnd_WithScheduledFire_ReturnsImmediately_WhenInitialStateAlreadyEnded()
    {
        ...
        Assert.Empty(recorded.Recording.Frames[0].State.Projectiles);
    ...
    public void RecordUntilEnd_WithScheduledFire_RecordsRequestAtMatchingTick()
    {
        ...
        MatchRecordedRunResult recorded = result.RecordedRunResult;
        Assert.Equal(2, recorded.RunResult.TicksExecuted);
        Assert.Equal(3, recorded.Recording.Frames.Count);

        Assert.Equal(new SimTick(0), recorded.Recording.Frames[0].State.CurrentTick);
        Assert.Empty(recorded.Recording.Frames[0].State.Projectiles);

        Assert.Equal(new SimTick(1), recorded.Recording.Frames[1].State.CurrentTick);
        Assert.Single(recorded.Recording.Frames[1].State.Projectiles);
        Assert.Equal(
            FixedVec2.FromInts(51, 50),
            recorded.Recording.Frames[1].State.Projectiles[0].Position);

        Assert.Equal(new SimTick(2), recorded.Recording.Frames[2].State.CurrentTick);
        Assert.Single(recorded.Recording.Frames[2].State.Projectiles);
        Assert.Equal(
            FixedVec2.FromInts(52, 50),
            recorded.Recording.Frames[2].State.Projectiles[0].Position);
        ...
        Assert.Equal(CombatLogEventTypes.MatchStarted, entries[0].EventType);
        Assert.Equal(CombatLogEventTypes.FireRequested, entries[1].EventType);
        Assert.Equal(CombatLogEventTypes.FireSucceeded, entries[2].EventType);
        Assert.Equal(CombatLogEventTypes.ProjectileSpawned, entries[3].EventType);
        Assert.Equal(CombatLogEventTypes.MatchEnded, entries[4].EventType);
        Assert.Equal(new SimTick(0), entries[0].Tick);
        Assert.Equal(new SimTick(0), entries[1].Tick);
        Assert.Equal(new SimTick(0), entries[2].Tick);
        Assert.Equal(new SimTick(0), entries[3].Tick);
        Assert.Equal(new SimTick(2), entries[4].Tick);
    }
    ...
    public void RecordUntilEnd_WithScheduledFire_DoesNotApplyFutureRequestBeforeTimeout()
    {
        ...
        for (int i = 0; i < recorded.Recording.Frames.Count; i++)
        {
            Assert.Empty(recorded.Recording.Frames[i].State.Projectiles);
        }
        ...
        Assert.Equal(new SimTick(0), entries[0].Tick);
        Assert.Equal(CombatLogEventTypes.MatchEnded, entries[1].EventType);
        Assert.Equal(new SimTick(2), entries[1].Tick);
    }
    ...
    public void RecordUntilEnd_WithScheduledFire_RecordsScheduledLethalFire()
    {
        ...
        MatchRecordedRunResult recorded = result.RecordedRunResult;
        Assert.Equal(1, recorded.RunResult.TicksExecuted);
        Assert.True(recorded.RunResult.EndCondition.IsEnded);
        Assert.Equal(MatchEndReason.TankDestroyed, recorded.RunResult.EndCondition.Reason);
        Assert.True(recorded.RunResult.EndCondition.WinnerTankId.HasValue);
        Assert.Equal(new TankId(0), recorded.RunResult.EndCondition.WinnerTankId!.Value);
        Assert.Equal(2, recorded.Recording.Frames.Count);

        Assert.Equal(new SimTick(0), recorded.Recording.Frames[0].State.CurrentTick);
        TankState frame0Tank1 = recorded.Recording.Frames[0].State.Tanks
            .First(t => t.Id == new TankId(1));
        Assert.Equal(20, frame0Tank1.CurrentHitPoints);
        Assert.Empty(recorded.Recording.Frames[0].State.Projectiles);

        Assert.Equal(new SimTick(1), recorded.Recording.Frames[1].State.CurrentTick);
        TankState frame1Tank1 = recorded.Recording.Frames[1].State.Tanks
            .First(t => t.Id == new TankId(1));
        Assert.Equal(0, frame1Tank1.CurrentHitPoints);
        Assert.Empty(recorded.Recording.Frames[1].State.Projectiles);
    ...
    [Fact]
    public void RecordUntilEnd_WithScheduledFire_LogOrderIsNonDecreasing()
    {
        MatchState initialState = CreateState(new SimTick(0));
        MatchFireRequest request = CreateRequest(
            muzzle: FixedVec2.FromInts(50, 50),
    ...
    public void RecordUntilEnd_WithScheduledFire_DoesNotMutateInitialState()
    {
        ...
        LoggedMatchReplayRecorder.RecordUntilEnd(state, maxTicks: 2, schedule);
